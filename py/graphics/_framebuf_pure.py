...
def _hline(fb, x, y, w, c):
    fb.fill_rect(x, y, w, 1, c)


def _vline(fb, x, y, h, c):
    fb.fill_rect(x, y, 1, h, c)


def _line(fb, x0, y0, x1, y1, c):
    dx = abs(x1 - x0)
...
def _rect(fb, x, y, w, h, c, f=False):
    if f:
        fb.fill_rect(x, y, w, h, c)
    else:
        _hline(fb, x, y, w, c)
        _hline(fb, x, y + h - 1, w, c)
        _vline(fb, x, y, h, c)
        _vline(fb, x + w - 1, y, h, c)


def _ellipse(fb, cx, cy, rx, ry, c, f=False, m=0x0F):
    if rx == 0 and ry == 0:
        fb.pixel(cx, cy, c)
        return
    x = rx
...
class FrameBuffer:
    ...
    def blit(self, *args, **kwargs):
        raise NotImplementedError("blit not implemented in pure fallback")

    def ellipse(self, x0, y0, r1, r2, c, f=False, m=0x0F):
        _ellipse(self, x0, y0, r1, r2, c, f, m)

    def hline(self, x0, y0, w, c):
        _hline(self, x0, y0, w, c)

    def line(self, x0, y0, x1, y1, c):
        _line(self, x0, y0, x1, y1, c)

    def poly(self, *args, **kwargs):
        raise NotImplementedError("poly not implemented in pure fallback")

    def rect(self, x0, y0, w, h, c, f=False):
        _rect(self, x0, y0, w, h, c, f)

    def vline(self, x0, y0, h, c):
        _vline(self, x0, y0, h, c)

    def text(self, *args, **kwargs):
        raise NotImplementedError("text not implemented in pure fallback")
